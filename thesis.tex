...
\pagebreak The currently accepted energy model is a multilinear model
articulated into three main parts:
%
\begin{equation}
...
\pagebreak Typically, energy profiling studies in wireless
communications provide a measurement setup that is expressly designed
for the electrical specifications of the device under test. For
instance, \citet{Feeney2001} and \citet{Jean-pierreEbert} use a cardbus
extender and an oscilloscope to measure wireless interfaces.
\citet{Carroll2010}, \citet{Rice2010}, \citet{Wang2010} perform energy
measurements on smartphones using a data acquisition (DAQ) card, while
\citet{gupta2007} used a power meter.
...
\begin{center}
\resizebox{\linewidth}{!}{\begin{tabular}{rrr}
\toprule
  & Mode Index & MCS [Mbps]\\
\midrule
 & 1 & 6\\
...
\begin{center}
\resizebox{\linewidth}{!}{\begin{tabular}{rlllcllc}
\toprule
...
\begin{center}
\resizebox{\linewidth}{!}{\begin{tabular}{>{\ttfamily}ll}
\toprule
Priority & Event\\
\midrule
...
\bookmarksetup{startatroot} \addtocontents{toc}{\partseparator}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{n <-}\StringTok{ }\NormalTok{1e4L}
...
\pagebreak This toy example performs a very simple simulation in which
\texttt{n} arrivals are attached (in one shot, thanks to the convenience
function \texttt{at()}) to a \texttt{test} trajectory at
\(t=1, 2, ..., n\). The trajectory consists of a single activity: a
timeout with some configurable \texttt{delay} that may be a fixed value
or a function call. Finally, after the simulation, the monitored data is
extracted from the simulation core to R. Effectively, this is equivalent
to generating a data frame of \texttt{n} rows (see the example output in
...
\NormalTok{    name[i] <-}\StringTok{ }\KeywordTok{paste0}\NormalTok{(}\StringTok{"test"}\NormalTok{, i}\OperatorTok{-}\DecValTok{1}\NormalTok{)}
...
\begin{center}
\resizebox{\linewidth}{!}{\begin{tabular}{>{\ttfamily}rrrrr}
\toprule
